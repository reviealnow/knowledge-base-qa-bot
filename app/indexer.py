...
class Section:
    ...
    tokens: list[str] # tokenized heading_path + content (no stop words)
...
# Module-level state — single process, reloaded on startup
...
def parse_markdown(path: Path) -> list[Section]:
    """Split one .md file into Section records at heading boundaries."""
    parsed: list[Section] = []
    ...
    def flush() -> None:
        ...
        full_text = " ".join(heading_path) + " " + content
...
def load_index(index_path: Path = INDEX_PATH) -> tuple[int, int]:
    global sections
...
def _write_index(index_path: Path = INDEX_PATH) -> None:
...
def bm25_score(query_tokens: list[str], section: Section, k1: float = 1.5, b: float = 0.75) -> float:
    ...
    # Boost when query terms appear in the heading path (exact heading match signal)
